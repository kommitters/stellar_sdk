...
It is possible to use different signature types settled by the `SetOptions` transaction into the network.

```elixir
# ed25519
# ed25519 keypair or ed25519 secret key.
signature = Stellar.TxBuild.Signature.new(signer_key_pair)
signature = Stellar.TxBuild.Signature.new(ed25519: "SGDC3W2X5KUTZRTQIKXM5D2I5WG5JYSEJQWEELVPQ5YMWZR6CA2JJ35RW")

# hash_x
# hash_x is the hex preimage value (x) generated by a random value of 256-bit.
signature = Stellar.TxBuild.Signature.new(hash_x: "dea550a5e5897ef98a37a828e183d8d313486159329df646c1137147e766282c")

# signed_payload - {payload, secret}
# payload is a hex string, maximum 32 bytes.
# secret is a ed25519 secret key.
signature = Stellar.TxBuild.Signature.new(signed_payload: {"01020304", "SACHJRYLY43MUXRRCRFA6CZ5ZW5JVPPR4CWYWIX6BWRAOHOFVPVYDO5Z"})
```

In both cases, a transaction may need more than one signature. If the transaction has operations that affect more than one account, it will need authorization from every account in question. A transaction will also need additional signatures if the account associated with the transaction has multiple public keys.
