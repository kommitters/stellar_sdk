defmodule Stellar.TxBuild.SCObjectTest do
  ...
  alias Stellar.TxBuild.{SCAddress, SCVal, SCMapEntry}
  alias StellarBase.XDR.{SCVec, SCMap}
  ...
  test "new/1 when type is vec", %{public_key: public_key} do
     %TxSCAddress{type: :account, value: ^public_key} = TxSCAddress.new(account: public_key)
  end

  test "new/1 when type is map", %{hash: hash} do
    %TxSCAddress{
      type: :contract,
      value: ^hash
